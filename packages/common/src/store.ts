...
export function defineStore<TStoreData extends TStoreBaseData>(
  storeName: string,
  userDataPath?: string,
): TStore<TStoreData> {
  ...
  const filePath = resolveStoreFilePath(storeName, userDataPath || resolveUserDataPath());
  const _cachedStore = JSON.parse(loadFromFs(filePath));

  function getValue<TKey extends keyof TStoreData>(key: TKey): TStoreData[TKey] {
    return _cachedStore[key];
  }

  function setValue<TKey extends keyof TStoreData>(key: TKey, value: TStoreData[TKey]) {
